...
export class SaveMasterFacilityHandler implements ICommandHandler<SaveMasterFacilityCommand> {
  constructor(
  ...
  async createMasterFacility(command: SaveMasterFacilityCommand): Promise<any> {
    const newMasterFacility = new MasterFacility(command._id, command.code, command.name, command.county, command.mechanism);
